...
### Exercises
#### Exercise 1: calculating logs
Complete the code below to calculate the log of 5.

```{r log, exercise=TRUE, exercise.eval=TRUE}
log
```
...
#### Exercise 3: calculating means
The code below creates a vector called X. Add an additional line of code to calculate the mean of X.

```{r mean-exercise, exercise=TRUE, exercise.eval=TRUE}
X <- c(1,3,6,8,9)
```
...
#### Exercise 4: calculating means when you have missing values
Add another argument to the mean function below to ignore the missing values in the the vector when calculating the mean.

```{r na-rm, exercise=TRUE, exercise.eval=TRUE}
VEC <- c(5,2,NA,7,8,4,7,NA)
mean(VEC)
```
...
#### Exercise 5: subsetting vectors
The code below creates a vector called Y. Add an additional line of code to create a new vector called Z that contains only the first two values of Y. Try using brackets to subset the Y vector, rather than just typing the values into a new vector.

```{r subset, exercise=TRUE, exercise.eval=TRUE}
Y <- c(3,5,6,1,7)
```
...
#### Exercise 6: extracting a column from a data frame
The code below creates a data frame called DATA that includes two variables (VARIABLE1 and VARIABLE2). Add a new line of code to extract the VARIABLE2 column from  the data frame and save it as a vector called V2.

```{r data-frame, exercise=TRUE, exercise.eval=TRUE}
DATA <- data.frame(VARIABLE1=c(1:3),VARIABLE2=c(5,6,8))
DATA
```
