...
We will once again start our civu example with a discrete (categorical) independent variable and a continuous dependent variable. 
...
Now let's make an equivalent plot for our alternative model. Because R likely classified our herbivory treatment variable as a numeric variable when we loaded the data, we will reclassify that variable as a factor before we make our graph. Then we will make a box plot, just as before, but this time including herbivory as our x variable.
